...
$F13::
{
	If (Layer=1) and WinActive("Discord")
	{
		...
		{
			;SetKeyDelay 10, 32
			SendInput, {LControl Down}{k}{LControl Up}{Enter}
			KeyWait, F13
		}
	}
	Else If (Layer=1) and WinActive("Skyrim Special Edition")
	{
		KeyWait F13, t0.333
		t:= A_TimeSinceThisHotkey
		If ErrorLevel
		{
			Send {SC029}
			SetKeyDelay 1, 1
			Send player.modav carryweight 500
			Send {Enter}{SC029}
			KeyWait, F13
		}
		Else
...
$F14::
{
	If (Layer=1) and WinActive("Skyrim Special Edition")
	{
		Send {SC029}
		SetKeyDelay 1, 1
		Send showracemenu
	...
	Else If (Layer=2) and WinActive("Skyrim Special Edition")
	{
		Send {esc Down}
		KeyWait, F14
		Send {esc Up}
