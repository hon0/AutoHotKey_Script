...
*SC005:: ; Air Fighter selection, See LButton Filter
{
	KeyWait, SC005, T0.1
	If (ErrorLevel)
	{
		If GetKeyState("MButton", "P")=1
		{
			Send {MButton Up}{SC005}{SC005 Down}{MButton Down} ; Select Phoenix
			FighterFilter := 2
			keywait SC005
			Send {SC005 Up}
		}
	...
	Else 
	{
		...
		If (ErrorLevel)
		{
			If GetKeyState("MButton", "P")=1
			{
				Send {MButton Up}{SC005}{SC005 Down}{MButton Down} ; Select HummmingBird
				FighterFilter := 1
				keywait SC005
			}
			Else
		...
		Else
		{
			If GetKeyState("MButton", "P")=1
			{
				Send {MButton Up}{SC005}{SC005 Down}{MButton Down} ; Select Avenger
				FighterFilter := 3
				keywait SC005
			}
			Else
...
~LButton Up:: ; Selection filter
{
	...
	Else	If FighterFilter = 2 ; Selection filter advanced fightery
	{
		Sleep 67
		Send {h}
	...
	Else	If FighterFilter = 1 ; Selection filter Orbital Fighter
	{
		Sleep 67
		Send {y}
		Send {LControl Down}{Numpad2}{LControl Up}
	...
	Else	If FighterFilter = 3 ; Selection filter Air Fighter
	{
		Sleep 67
		Send {y}
	...
	Else	If BomberFilter = 2 ; Selection filter Air Fighter
	{
		Sleep 67
		Send {LControl Down}{Numpad2}{LControl Up}u{j}
		BomberFilter := 0
		Return
	}
	Else	If BomberFilter = 1 ; Selection filter Air Fighter
	{
		Sleep 67
		Send {LControl Down}{Numpad2}{LControl Up}{u}
		BomberFilter := 0
		Return
	}
	Else	If BomberFilter = 3 ; Selection filter Air Fighter
	{
		Sleep 67
		Send {LControl Down}{Numpad2}{LControl Up}{k}
		BomberFilter := 0
		Return
	}
	Return
