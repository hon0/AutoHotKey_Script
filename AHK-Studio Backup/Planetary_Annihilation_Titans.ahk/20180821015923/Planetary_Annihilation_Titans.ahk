...
$SC00D:: ; Air Fighter selection
{
	...
	If (ErrorLevel)
	{
		If GetKeyState("MButton", "P")=1
		{
			Send {MButton Up}{SC00D}{SC00D}{MButton Down} ; Select Advanced Fighter
			FighterFilter := 2
			keywait SC00D
		}
		Else
	...
	Else 
	{
		KeyWait, SC00D, D T0.1
		If (ErrorLevel)
		{
			If GetKeyState("MButton", "P")=1
			{
				Send {MButton Up}{SC00D}{SC00D}{MButton Down} ; Select Air and Oribtal Fighter
				FighterFilter := 1
				keywait SC00D
			}
			Else
		...
		Else
		{
			If GetKeyState("MButton", "P")=1
			{
				Send {MButton Up}{SC00D}{SC00D}{MButton Down} ; Select Orbital Fighter
				FighterFilter := 3
				keywait SC00D
			}
			Else
...
~LButton Up:: ; Selection filter
{
	...
	{
		Send {LControl Down}{LAlt Down}{Numpad4}{LAlt Up}{Numpad5}{LControl Up}{h}
		FighterFilter := 0
		Return
	}
