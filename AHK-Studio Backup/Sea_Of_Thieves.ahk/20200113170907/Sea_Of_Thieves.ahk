...
~z::
If (z_pressed) or (d_pressed)
	return
z_pressed := 1
If (Toggle_Walk = 1) and (Moving = 0) and (z_pressed = 1)
{
	Sleep 64
	SendInput {LShift}
}
...
~d::
If (d_pressed) or (z_pressed)
	return
d_pressed := 1
If (Toggle_Walk = 1) and (Moving = 0) and (d_pressed = 1)
{
	Sleep 64
	SendInput {LShift}
}
