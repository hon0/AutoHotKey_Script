...
#IfWinActive ahk_class LaunchUnrealUWindowsClient ; Fable Anniversary
	{
		$F24::
		{
			If (F24_pressed)
		...
		$Insert::
		{
			If (Layer=1) and GetKeyState("LButton")
			{
				Send {WheelUp}
				Return
			}
		...
		$*SC002::
		{
			...
			If (Layer=1)
			{
				SendInput {Blind}{SC002 Down}
			}
			If (Layer=2)
			{
				SendInput {Blind}{F1 Down}
			}
			Return
		}
		...
		$*SC004::
		{
			...
			If (Layer=1)
			{
				SendInput {Blind}{SC004 Down}
			}
			If (Layer=2)
			{
				SendInput {Blind}{F3 Down}
			}
			Return
		}
